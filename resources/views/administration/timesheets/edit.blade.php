...
@section('title', 'Modifier la saisie de temps - ProjEct IT')
@section('page_title', 'Saisies de temps')
@section('page_subtitle', 'Modifier la déclaration #' . $timesheet->id)
...
@section('content')
    ...
    @if(session('error'))
        <div class="max-w-xl mx-auto mb-4 p-3 bg-red-50 border border-red-200/80 rounded-md flex items-center justify-between text-xs text-red-800">
    ...
    <div class="max-w-xl mx-auto bg-white rounded-lg border border-slate-200/80 shadow-sm overflow-hidden">
        <div class="p-5 border-b border-slate-100 bg-slate-50/50">
            <h2 class="text-xs font-semibold uppercase tracking-wider text-slate-700">Éditer la saisie de temps</h2>
            <p class="text-[11px] text-slate-500 mt-0.5">Mettez à jour les informations enregistrées pour cette intervention.</p>
        </div>

        <form action="{{ route('administration.timesheets.update', $timesheet) }}" method="POST" class="p-6 space-y-4">
            @csrf
            @method('PUT')
            ...
            <!-- Sélection du Membre -->
            <div>
                <label class="block text-xs font-semibold text-slate-700 mb-1.5">Membre de l'équipe <span class="text-emerald-600">*</span></label>
                <select name="user_id" required class="w-full bg-slate-50/50 border border-slate-200 rounded-md px-3 py-2 text-xs text-slate-900 focus:bg-white focus:border-emerald-500 focus:ring-1 focus:ring-emerald-500 outline-none transition">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    <input type="date" name="logged_at" value="{{ old('logged_at', \Carbon\Carbon::parse($timesheet->logged_at)->format('Y-m-d')) }}" required class="w-full bg-slate-50/50 border border-slate-200 rounded-md px-3 py-2 text-xs text-slate-900 focus:bg-white focus:border-emerald-500 focus:ring-1 focus:ring-emerald-500 outline-none transition">
                    @error('logged_at') 
                        <p class="text-[11px] text-red-500 mt-1 font-medium">{{ $message }}</p> 
                    @enderror
                </div>
            ...
            <div>
                ...
                <textarea name="note" rows="3" placeholder="Détails des travaux effectués..." class="w-full bg-slate-50/50 border border-slate-200 rounded-md px-3 py-2 text-xs text-slate-900 placeholder:text-slate-400 focus:bg-white focus:border-emerald-500 focus:ring-1 focus:ring-emerald-500 outline-none transition">{{ old('note', $timesheet->note) }}</textarea>
                @error('note') 
                    <p class="text-[11px] text-red-500 mt-1 font-medium">{{ $message }}</p> 
                @enderror
            </div>
